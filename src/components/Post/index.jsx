...
const Post = ({ tweet }) => {
  // kullanıcı edit modunda mı ?
  const [isEditMode, setIsEditMode] = useState(false);

  // tweet'in atılma tarihini hesaplama
  const date = moment(tweet?.createdAt?.toDate()).fromNow();

  // aktif kullanıcı bu tweeti likeladı mı sorgusu
  ...
  const handleLike = async () => {
    ...
    await updateDoc(ref, {
      likes: isLiked // bu kullanıcı tweet'i likeladı mı
        ? arrayRemove(auth.currentUser.uid) // like atmışsa like kaldır
        : arrayUnion(auth.currentUser.uid), // like atmadıysa like at
    });
  };
